...
const inputClass =
  "w-full rounded-xl border border-black/10 bg-white px-3.5 py-3 text-sm text-neutral-900 outline-none transition focus:border-neutral-500";
const labelClass =
  "mb-1.5 block text-[11px] font-medium uppercase tracking-[0.14em] text-neutral-500";
...
function Toggle({
  ...
  return (
    <label className="flex cursor-pointer items-center justify-between gap-4 rounded-xl border border-black/10 px-4 py-3">
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-neutral-950 p-6 text-white">
        <form onSubmit={login} className="w-full max-w-sm rounded-3xl border border-white/10 bg-white/5 p-8">
          ...
          <button className="w-full rounded-xl bg-white px-4 py-3 text-sm font-medium text-neutral-950">
            Увійти
          </button>
        </form>
      </div>
    ...
    if (section === "hero") {
      return (
        <div className="space-y-4">
          ...
          <Field label="Кнопка" value={content.hero.cta} onChange={(value) => change((draft) => { draft.hero.cta = value; })} />
      ...
      return (
        <div className="space-y-5">
          {content.advantages.map((item, index) => (
            <div key={index} className="space-y-3 rounded-2xl border border-black/10 p-4">
      ...
      <div className="grid h-full grid-cols-[190px_minmax(0,1fr)_380px]">
        <aside className="flex flex-col border-r border-black/10 bg-neutral-950 p-4 text-white">
          <div className="mb-7 px-2 pt-2">
            ...
            {previewOnly ? (
              <span className="mt-2 inline-flex rounded-full bg-amber-400/15 px-2.5 py-1 text-[10px] font-medium uppercase tracking-[0.14em] text-amber-300">
                Demo mode
              </span>
            ) : null}
          </div>
          ...
          <button
            onClick={logout}
            className="mt-auto inline-flex items-center gap-2 rounded-xl px-3 py-2.5 text-sm text-white/60 hover:bg-white/10 hover:text-white"
          >
            <LogOut className="size-4" />
            {previewOnly ? "На сайт" : "Вийти"}
          </button>
        </aside>
              ...
              <div className="flex h-16 items-center justify-between border-b border-black/10 bg-white px-5">
                ...
                {previewOnly ? (
                  <span className="rounded-full bg-amber-50 px-2.5 py-1 text-[11px] font-medium text-amber-700">
                    Demo: реальні заявки зʼявляться на хостингу
                  </span>
                ) : null}
              </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button
                    ...
                    className={`inline-flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-medium text-white disabled:opacity-50 ${
                      previewOnly ? "bg-neutral-500" : "bg-neutral-950"
                    }`}
                  >
                    {saving ? <Loader2 className="size-4 animate-spin" /> : <Save className="size-4" />}
                    {previewOnly ? "Перевірити" : "Опублікувати"}
                  </button>
                </div>
              </div>
