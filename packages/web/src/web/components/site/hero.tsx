...
export function Hero() {
  return (
    <section className="relative overflow-hidden pt-[112px] md:pt-[140px]">
      <div
        aria-hidden
        className="pointer-events-none absolute -right-40 -top-40 size-[620px] rounded-full bg-[radial-gradient(circle,rgba(162,152,141,0.22),transparent_65%)]"
      />

      <div className="container-x relative grid items-end gap-12 pb-16 lg:grid-cols-[1.05fr_0.95fr] lg:gap-20 lg:pb-24">
        <div className="flex flex-col gap-8">
          ...
          <h1 className="reveal font-display leading-[0.88]" style={{ ["--reveal-delay" as string]: "80ms" }}>
            <span className="block text-[clamp(3.5rem,11vw,8.5rem)] italic text-ink">
              {hero.titleTop}
            </span>
            <span className="block text-[clamp(2rem,5.4vw,4.2rem)] uppercase tracking-[0.14em] text-taupe">
              {hero.titleAccent}
            </span>
          </h1>

          <p
            className="reveal max-w-xl text-[15px] leading-relaxed text-ink-soft md:text-base"
          ...
          >
            <button
              type="button"
              onClick={() => go("#courses")}
              className="group inline-flex items-center gap-3 rounded-full bg-ink px-8 py-4 text-[11px] uppercase tracking-[0.2em] text-cream transition-colors duration-300 hover:bg-taupe-deep"
            >
              {hero.cta}
            ...
            <button
              ...
              className="rounded-full border border-taupe px-8 py-4 text-[11px] uppercase tracking-[0.2em] text-taupe-deep transition-colors duration-300 hover:bg-sand"
            >
              {hero.ctaSecondary}
            </button>
          </div>
      ...
      <div className="border-y border-linen bg-terracotta py-3.5">
